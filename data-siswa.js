const studentData = {
    Refcy: {
        city: 'Bandung',
        instagram: 'Belum diisi',
        quote: 'Terus belajar dan tetap kompak bersama teman-teman.'
    },
    Arizkar: {
        city: 'Belum diisi',
        instagram: 'Belum diisi',
        quote: 'Setiap proses kecil membawa kita lebih dekat ke mimpi.'
    },
    Galih: {
        city: 'Bandung',
        instagram: 'galihardiansyahh',
        quote: 'Nikmati perjalanan, karena kenangan kelas tidak terulang.',
        photo: 'assets/foto-siswa/galih.jpg'
    },
    Indra: {
        city: 'Bandung',
        instagram: 'Belum diisi',
        quote: 'Kita mungkin berbeda, tetapi tujuan kita tetap sama.'
    },
    Rafa: {
        city: 'Bandung',
        instagram: 'Belum diisi',
        quote: 'Berani mencoba adalah langkah pertama menuju berhasil.'
    },
    Dian: {
        city: 'Subang',
        instagram: 'Belum diisi',
        quote: 'Jangan takut bermimpi besar.'
    },
    Fasya: {
        city: 'Purwakarta',
        instagram: 'Belum diisi',
        quote: 'Kebersamaan adalah kekuatan terbaik kelas kita.'
    },
    Ritchie: {
        city: 'Subang',
        instagram: 'Belum diisi',
        quote: 'Hari ini belajar, besok menjadi lebih baik.'
    },
    Hilmi: {
        city: 'Bandung',
        instagram: 'Belum diisi',
        quote: 'Tetap rendah hati dan terus melangkah.',
        photo: 'assets/foto-siswa/hilmi.jpg'
    },
    Panji: {
        city: 'Bandung',
        instagram: 'Belum diisi',
        quote: 'Tetap semangat dan terus melangkah bersama kelas.',
        photo: 'assets/foto-siswa/panji.jpg'
    },
    Fatimah: {
        city: 'Malang',
        instagram: 'Belum diisi',
        quote: 'Kelas adalah tempat cerita indah dimulai.'
    },
    Nayla: {
        city: 'Cimahi',
        instagram: 'Belum diisi',
        quote: 'Jalani hari dengan semangat dan senyuman.'
    },
    Putri: {
        city: 'Cimahi',
        instagram: 'Belum diisi',
        quote: 'Mimpi besar dimulai dari langkah sederhana.'
    },
    Suci: {
        city: 'Bandung',
        instagram: 'Belum diisi',
        quote: 'Bersama kita bisa melewati semuanya.'
    },
    Azizah: {
        city: 'Belum diisi',
        instagram: 'Belum diisi',
        quote: 'Jadilah versi terbaik dari dirimu sendiri.'
    },
    Famella: {
        city: 'Belum diisi',
        instagram: 'Belum diisi',
        quote: 'Simpan kenangan baik, bawa semangatnya.'
    },
    Adinda: {
        city: 'Belum diisi',
        instagram: 'Belum diisi',
        quote: 'Tidak ada yang tidak mungkin jika dilakukan bersama.'
    },
    Feliya: {
        city: 'Belum diisi',
        instagram: 'Belum diisi',
        quote: 'Terus tumbuh, terus bersinar.'
    },
    Alya: {
        city: 'Belum diisi',
        instagram: 'Belum diisi',
        quote: 'Kebahagiaan sederhana selalu layak dirayakan.'
    },
    Sherly: {
        city: 'Belum diisi',
        instagram: 'Belum diisi',
        quote: 'Berikan yang terbaik untuk setiap kesempatan.'
    },
    Maryam: {
        city: 'Belum diisi',
        instagram: 'Belum diisi',
        quote: 'Teman seperjuangan adalah hadiah terbaik di sekolah.'
    },
    Fajriany: {
        city: 'Belum diisi',
        instagram: 'Belum diisi',
        quote: 'Tetap semangat mengejar cita-cita.'
    }
};
